From e458e74d504878da6b3818ba611ba3d27647a67c Mon Sep 17 00:00:00 2001
From: Qiang Rao <qiang.rao@intel.com>
Date: Wed, 20 Jan 2021 17:40:12 +0800
Subject: [PATCH 1086/1087] tcc: update RTCT table parser to support two
 versions
...
---
 ...
 drivers/tcc/tcc_buffer.c    | 540 ++++++++++++++++++++++++++++++------
 include/acpi/actbl2.h       |  44 ++-
 3 files changed, 545 insertions(+), 90 deletions(-)

diff --git a/arch/x86/kernel/acpi/boot.c b/arch/x86/kernel/acpi/boot.c
index 00010b4cb947..968b5d27eb18 100644
--- a/arch/x86/kernel/acpi/boot.c
+++ b/arch/x86/kernel/acpi/boot.c
@@ -1239,6 +1239,10 @@ static inline int acpi_parse_madt_ioapic_entries(void)
...
+
 ...
 {
 	int i;
@@ -1265,13 +1269,32 @@ static int __init acpi_parse_ptct(struct acpi_table_header *table)
...
+	/* Parse PTCT table for the second round to get number of regions*/
 	...
 	ptr = (u8 *)table;
 	ptr += PTCT_ACPI_HEADER_SIZE;
@@ -1280,15 +1303,18 @@ static int __init acpi_parse_ptct(struct acpi_table_header *table)
 		entry = (struct acpi_ptct_entry_header *)(ptr);
 		offset += entry->size;
...
+	/* Parse for the third round to record address for each regions */
 	...
 	ptr = (u8 *)table;
 	ptr += PTCT_ACPI_HEADER_SIZE;
@@ -1297,11 +1323,20 @@ static int __init acpi_parse_ptct(struct acpi_table_header *table)
 		entry = (struct acpi_ptct_entry_header *)(ptr);
 		offset += entry->size;
...
@@ -1311,7 +1346,7 @@ static int __init acpi_parse_ptct(struct acpi_table_header *table)
...
index a9f83c1d0722..aa15e071594c 100644
...
+static u32 ssram_waymask_v2[2][MAXCACHEID] = {{0}};
 ...
 struct tcc_ptct_entry_header {
 	u16 size;
@@ -120,12 +147,44 @@ struct tcc_ptct_mhlatency {
 	u32 *apicids;
 };
...
@@ -149,22 +208,54 @@ struct tcc_config {
...
@@ -206,10 +297,17 @@ static void tcc_get_cache_info(void)
 	} while ((eax & 0x1F) != 0);
 }
...
@@ -218,19 +316,28 @@ static void tcc_get_psram_cpumask(u32 apicid, u32 num_threads_sharing, cpumask_t
...
@@ -243,24 +350,84 @@ static int tcc_parse_ptct(void)
...
+	/* Parse and save ssram regions */
 	...
 	offset = ACPI_HEADER_SIZE;
 	tbl_swap = tbl_swap + offset;
@@ -271,54 +438,127 @@ static int tcc_parse_ptct(void)
 		entry_size = entry_header->size;
 		entry_type = entry_header->type;
...
@@ -334,6 +574,8 @@ static int tcc_parse_ptct(void)
...
@@ -351,6 +593,9 @@ static int tcc_parse_ptct(void)
...
@@ -363,9 +608,18 @@ static int tcc_parse_ptct(void)
 					return -ENOMEM;
...
@@ -398,18 +652,32 @@ static u32 tcc_allocate_memslot(u32 id, size_t size)
 		}
 	}
...
@@ -417,67 +685,113 @@ static u32 tcc_allocate_memslot(u32 id, size_t size)
...
+	int cpu, testmask = 0;
 	...
 	if (p_tcc_config->minor == MINOR(i->i_rdev))
 		return 0;
@@ -490,7 +804,20 @@ static int tcc_buffer_open(struct inode *i, struct file *f)
...
@@ -503,6 +830,9 @@ static int tcc_buffer_close(struct inode *i, struct file *f)
 		return 0;
...
@@ -521,8 +851,17 @@ static int tcc_buffer_mmap(struct file *f, struct vm_area_struct *vma)
 		return -EINVAL;
 	}
...
@@ -540,6 +879,8 @@ static long tcc_buffer_ioctl(struct file *filp, unsigned int cmd, unsigned long
...
+
 	...
 	case TCC_GET_REGION_COUNT:
 		if (NULL == (int *)arg) {
@@ -616,7 +957,34 @@ static long tcc_buffer_ioctl(struct file *filp, unsigned int cmd, unsigned long
 		if (ret != 0)
 			return -EFAULT;
...
+		}
 		...
 		if (req_mem.devnode == UNDEFINED_DEVNODE)
 			return -ENOMEM;
@@ -649,8 +1017,10 @@ static void tcc_cleanup(void)
...
+				}
 				...
 				kfree(p_slot);
@@ -672,13 +1042,17 @@ static int __init tcc_buffer_init(void)
...
+		return -ENOMEM;
 	...
 	ret = tcc_parse_ptct();
@@ -692,6 +1066,8 @@ static int __init tcc_buffer_init(void)
 	}
...
+
 	...
 	if (ret < 0) {
 		pr_err("Unable to obtain a new minor number\n");
@@ -735,10 +1111,12 @@ static void __exit tcc_buffer_exit(void)
 	if (tcc_init) {
 		tcc_cleanup();
...
index f8b20148456d..0eac5b59338f 100644
...
@@ -1604,7 +1605,48 @@ struct ptct_psram_region {
