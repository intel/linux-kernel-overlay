...
%endif
%define embargoname 0405.intel_next
...
Source0: https://emb-overlay-koji.ostc.intel.com/tarball/kernel-next/intel-5.12-rc6-2021-04-05.tar.gz
Source1: overlay.config
...
Patch0001: 0001-x86-microcode-Force-update-a-uCode.patch
Patch0002: 0002-Add-ptusys-driver.patch
#Patch0003: 0003-Update-Socperf-driver-based-on-EMON-release-11-06-20.patch
...
%kernel_variant_package client
%description client-core
The kernel package contains the Linux kernel

%define variant_summary The Linux kernel server
%kernel_variant_package server
...
mv intel-5.* linux-%{KVERREL}

cd linux-%{KVERREL}

ApplyOptionalPatch 0001-x86-microcode-Force-update-a-uCode.patch
ApplyOptionalPatch 0002-Add-ptusys-driver.patch
#ApplyOptionalPatch 0003-Update-Socperf-driver-based-on-EMON-release-11-06-20.patch
...
# Copy config files
cp ../arch/x86/configs/intel_next_rpm_defconfig client.config
cp ../arch/x86/configs/dcg_x86_64_defconfig     server.config
...
%if %{with_up}
# Intel-Next first server then client
BuildKernel %make_target %kernel_image server
BuildKernel %make_target %kernel_image client
%endif
...
%kernel_variant_preun client
%kernel_variant_post -v client

%kernel_variant_preun server
%kernel_variant_post -v server
...
%ifnarch %nobuildarches
%kernel_variant_files 0 1 server
%kernel_variant_files 0 1 client
%endif
